...
class Ship:
    ...
    def set_thruster(self, on):
        if on:
            print "set thruster to true"
            self.thrust = True
            sound = ship_thrust_sound
            sound.play()
        else:
            print "set thruster to false"
            self.thrust = False
            sound = ship_thrust_sound
            sound.pause()
...
def draw(canvas):
    ...
    # lives and score
    canvas.draw_text('LIVES LEFT : ' + str(lives), (20, 30), 20, "White")
    canvas.draw_text('SCORE : ' + str(score), (680, 30), 20, "White")
